var logger = require('pomelo2-logger').getLogger('pomelo-rpc', 'InputBuffer');
...
InputBuffer.prototype.readObject = function() {
	...
	if (typeMap['null'] == type) {
		...
		if(!bean) {
			logger.error('readBean bean not found', {id});
			return;
		}
		...
		logger.error('readObject invalid read type', {type});
	}
