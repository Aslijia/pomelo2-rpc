var logger = require('pomelo2-logger').getLogger('pomelo-rpc', 'mqtt-mailbox');
  ...
  if (this.lastPing > 0) {
    if (this.lastPong < this.lastPing) {
      if (now - this.lastPing > KEEP_ALIVE_TIMEOUT) {
        logger.error('mqtt rpc client checkKeepAlive timeout from remote server',{ serverId:this.serverId, id:this.id, KEEP_ALIVE_TIMEOUT, lastPing:this.lastPing, lastPong:this.lastPong});
...
var setCbTimeout = function (mailbox, id, tracer, cb) {
  var timer = setTimeout(function () {
    ...
    var eMsg = util.format(' %d, remote server %s host: %s, port: %s', mailbox.serverId, mailbox.timeoutValue, id, mailbox.host, mailbox.port);
    logger.error("rpc callback timeout",{mailbox,id});
