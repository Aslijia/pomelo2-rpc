var logger = require('pomelo2-logger').getLogger('pomelo-rpc', 'ws-mailbox');
...
pro.connect = function (tracer, cb) {
  ...
  this.socket.on('message', function (pkg) {
    try {
      ...
      logger.error('rpc client process message with error: %s', { stack: err.stack } );
    }
  });
  ...
  this.socket.on('error', function (err) {
    logger.error('rpc socket is error', {host:self.host, port:self.port});
    self.emit('close', self.id);
    cb(err);
  });

  this.socket.on('disconnect', function (reason) {
    logger.error('rpc socket is disconnect', {reason});
...
var setCbTimeout = function (mailbox, id, tracer, cb) {
  var timer = setTimeout(function () {
    logger.warn('rpc request is timeout', {mailbox,id});
    clearCbTimeout(mailbox, id);
    if (!!mailbox.requests[id]) {
      delete mailbox.requests[id];
    }
    logger.error('rpc callback timeout', {mailbox});
...
var clearCbTimeout = function (mailbox, id) {
  if (!mailbox.timeout[id]) {
    logger.warn('timer is not exsits', {mailbox,id});
    return;
  }
