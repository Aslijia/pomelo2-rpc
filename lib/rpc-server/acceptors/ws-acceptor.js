var logger = require('pomelo2-logger').getLogger('pomelo-rpc', 'ws-acceptor');
...
pro.listen = function(port) {
  ...
  this.server.server.on('error', function(err) {
    logger.error('rpc server error',{stack: err.stack } );
    self.emit('error', err);
  });
  ...
  this.server.sockets.on('connection', function(socket) {
    ...
    socket.on('message', function(pkg) {
      try {
        ...
        logger.error('rpc server process message error: %j', {stack: err.stack } );
      }
    });
...
var ipFilter = function(obj) {
  if (typeof this.whitelist === 'function') {
    ...
    self.whitelist(function(err, tmpList) {
      if (err) {
        logger.error('%j.(RPC whitelist).', {err:err.message});
        return;
      }
      ...
      if (!!obj && !!obj.ip && !!obj.id) {
        ...
        if (sock) {
          ...
          logger.warn('%s is rejected(RPC whitelist).', { ip: obj.ip });
        }
      }
    });
  }
...
pro.close = function() {
  ...
  try {
    ...
    logger.error('rpc server close error: %j', {stack: err.stack } );
  }
  this.emit('closed');
};
